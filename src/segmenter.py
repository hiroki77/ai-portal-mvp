...
class TopicSegmenter:
    ...
    def _detect_silence_gaps(self, subs):
        ...
        for i in range(1, len(subs)):
            ...
            if gap >= self.SILENCE_GAP:
                gaps.append({
                    "after_time": round(subs[i - 1].end, 1),
    ...
    def _phase1_topics(self, subs):
        ...
        if gaps:
            gap_lines = [
                f"  {g['after_time']}s〜{g['before_time']}s ({g['gap_sec']}秒の無音)"
                for g in gaps[:20]
            ]
            gap_info = "\n\n【無音区間（話題の切れ目の強いヒント）】\n" + "\n".join(gap_lines)
        ...
        prompt = (
            ...
            "- 無音区間・転換ワード（じゃあ/次/ちなみに）を境界のヒントにする\n"
            "- 1話題の最低時間: 15秒\n"
            "- 動画全体を隙間なくカバーする\n\n"
            f"【字幕】\n{transcript}"
            f"{gap_info}\n\n"
            ...
            try:
                ...
                if valid:
                    ...
                    for i, t in enumerate(valid):
                        logger.info(
                            f"  [{i}] {t['start']:.0f}-{t['end']:.0f}s "
                            f"「{t['summary']}」 オチ:{t['punchline'][:20] if t['punchline'] else 'なし'}"
                        )
                    return valid
    ...
    # ===== Phase 2: 話題リストから最高クリップを選定 =====

    def _merge_short_topics(self, topics):
        """min_duration未満の話題を隣接と合算"""
        if not topics:
            return topics
        merged = list(topics)
        changed = True
        while changed:
            ...
            while i < len(merged):
                ...
                if dur < self.mn and i + 1 < len(merged):
                    nxt = merged[i + 1]
                    combined = nxt["end"] - t["start"]
                    if combined <= self.mx * 1.5:
                        new.append({
                            "start": t["start"],
                            "end": nxt["end"],
    ...
    def _phase2_select(self, topics, subs, pref=None, af=None):
        """Phase2: 話題リストから最高N本を選定。start/endは話題の値をそのまま使う"""
        merged = self._merge_short_topics(topics)
        candidates = [
            (i, t) for i, t in enumerate(merged)
            if t["end"] - t["start"] >= self.mn
        ] or list(enumerate(merged))
        ...
        prompt = (
            ...
            "JSON配列のみ出力（説明不要）:\n"
            '[{"index": 話題のindex番号, "score": 1-10, "reason": "選んだ理由（30文字以内）"}]'
        )
        ...
        for attempt in range(3):
            try:
                ...
                for sel in selected[:self.cc]:
                    ...
                    if en - st > self.mx:
                        st = en - self.mx
                    clip_subs = [sub for sub in subs if sub.start >= st and sub.end <= en + 1.0]
                ...
                if clips:
                    for i, c in enumerate(clips):
                        logger.info(
                            f"  Clip{i + 1}: {c.start:.0f}-{c.end:.0f}s({c.duration:.0f}s) "
                            f"score:{c.score} 「{c.topic_summary}」"
                        )
                    return clips
            except Exception as e:
    ...
    def _two_phase(self, subs, pref=None, af=None):
        ...
        return self._phase2_select(topics, subs, pref, af)

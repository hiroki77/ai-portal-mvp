"""processor v2
- 動的ズーム: テロップ位置を検出して必要最小限のズーム
- フォント精度: 元動画からサイズ・位置を自動検出
- 並列処理: 3クリップを並列で生成
"""
...
class VideoProcessor:
    ...
    def _detect_optimal_zoom(s,seg_path,w,h):
        """テロップの位置を検出して必要最小限のズーム率を計算"""
        if cv2 is None:
            return s.zoom_default
        try:
            # 動画から3フレーム取得してテロップ位置を検出
            ...
            # テロップの上端が見えなくなるズーム率を計算
            margin=10  # 余裕px
            text_from_bottom=h-top_text_y+margin
            zoom=h/(h-text_from_bottom)
            zoom=max(1.05,min(zoom,1.30))  # 1.05～1.30の範囲
        ...
        try:
            ...
            for fname in sorted(os.listdir(fd)):
                ...
                for cnt in contours:
                    ...
                    if ch_>10 and cw>20:  # ノイズ除外
                        text_heights.append(ch_)
                        text_y_positions.append(sub_h-y_-ch_)  # 下からの距離
            shutil.rmtree(fd,ignore_errors=True)

            if text_heights:
                import statistics
                med_h=statistics.median(text_heights)
                # フォントサイズ推定 (ピクセル高さ ≈ フォントサイズ * 0.75)
    ...
    def _write_ass(s,subs,clip_start,path,w,h,font_info):
        ...
        ac=s._rgb2ass(s.sub_cfg["aya_color"])
        jc=s._rgb2ass(s.sub_cfg["junpei_color"])
        sc=s._rgb2ass(s.sub_cfg["stroke_color"])
        header=(
            f"[Script Info]\nScriptType: v4.00+\nPlayResX: {w}\nPlayResY: {h}\n"
